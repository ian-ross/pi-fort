...
- cfdbedb: Use conversation messages instead of system prompt for fort hints.

  The fort context hint is now a regular message added once per session, instead of being injected via `before_agent_start`. This avoids invalidating the prompt cache when toggling fort on/off mid-session.
...
- 468b85f: Initial release of pi-fort: VM-isolated fort with automatic secret protection.

  - All tools (bash, read, write, edit) execute inside a Gondolin micro-VM
  - Modular drop-in config files (`pi-fort.d/`): git, jj, GitHub ship by default
  ...
  - `/fort init`, `/fort on|off`, `/fort add` with interactive search
